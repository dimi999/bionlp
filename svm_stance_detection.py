...
class SVMStanceDetector:
    """
    SVM-based stance detection model for COVID-19 vaccine tweets.
    
    This class implements Support Vector Machine approach to classify
    ...
    def __init__(self, data_path='merged_covid_vaccine_tweets.csv'):
        """Initialize the SVM stance detector with data loading and preprocessing."""
    ...
    def preprocess_data(self):
        ...
        plt.tight_layout()
        plt.savefig('svm_data_exploration.png', dpi=300, bbox_inches='tight')
        plt.show()
    
    def prepare_features(self):
        """Prepare feature vectors for SVM training."""
        print("Preparing features for SVM training...")
    ...
    def train_svm(self, kernel='rbf', C=1.0, gamma='scale'):
        ...
        Args:
            kernel (str): SVM kernel type ('rbf', 'linear', 'poly', 'sigmoid')
    ...
    def evaluate_svm(self):
        ...
        if self.svm_model is None:
            print("No SVM model trained yet! Please train the model first.")
            return None
    ...
    def predict_stance(self, tweet_text):
        """
        Predict stance for a new tweet using the trained SVM model.
        ...
        probabilities = self.svm_model.predict_proba(X_new)[0]
        prob_dict = {
            'Against': probabilities[0],
            'Neutral': probabilities[1],
            'Favor': probabilities[2]
        }
    ...
    def generate_svm_report(self):
        """Generate a comprehensive SVM model performance report."""
        if self.svm_model is None:
            print("No SVM model trained yet! Please train the model first.")
            return None
        
        print("Generating SVM model report...")
        ...
        report = {
            ...
            'svm_model_info': {
                ...
                'support_vectors': self.svm_model.n_support_.tolist()
            },
            'performance': {
                'accuracy': accuracy,
        ...
        # Save report
        with open('svm_model_report.json', 'w') as f:
            json.dump(report, f, indent=2, default=str)
        
        print("SVM report saved to 'svm_model_report.json'")
        
        return report

def main():
    """Main function to demonstrate the SVM stance detection system."""
    print("COVID-19 Vaccine Stance Detection - SVM Model")
    print("="*50)
    
    # Initialize the SVM detector
    svm_detector = SVMStanceDetector()
    
    # Train SVM with default parameters
    print("\nTraining SVM with default parameters...")
    accuracy = svm_detector.train_svm()
    
    # Evaluate the model
    print("\nEvaluating SVM model...")
    svm_detector.evaluate_svm()
    
    # # Perform hyperparameter tuning
    # print("\nPerforming hyperparameter tuning...")
    # best_params = svm_detector.hyperparameter_tuning()
    ...
    report = svm_detector.generate_svm_report()
    ...
    print("\n" + "="*50)
    print("Testing SVM with Sample Tweets:")
    print("="*50)
    
    for i, tweet in enumerate(sample_tweets, 1):
        print(f"\nSample {i}: {tweet}")
        result = svm_detector.predict_stance(tweet)
        if result:
    ...
    print("\n" + "="*50)
    print("SVM Training Complete! You can now use svm_detector.predict_stance() method")
